namespace Adept_AIO.Champions.Ezreal.Update.OrbwalkingEvents
{
    ...
    internal class LaneClear
    {
        public static void OnUpdate()
        {
            ...
            {
                var minion = GameObjects.EnemyMinions.FirstOrDefault(x => x.Health < Global.Player.GetSpellDamage(x, SpellSlot.Q) && x.IsValidTarget(SpellConfig.Q.Range) && x.Distance(Global.Player) > Global.Player.AttackRange);
                if (minion == null)
                {
                    return;
                }
