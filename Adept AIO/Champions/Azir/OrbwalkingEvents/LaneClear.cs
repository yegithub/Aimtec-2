        ...
        public static void OnUpdate()
        {
                ...
                Global.Player.ManaPercent() > MenuConfig.Lane["Q"].Value)
            {
                var minion = GameObjects.EnemyMinions.LastOrDefault(x =>
                    x.Distance(Global.Player) <= SpellConfig.Q.Range && x.MaxHealth > 10 && x.IsValid && !x.IsDead);
                if (minion == null)
                {
                    return;
                }
                ...
                AzirHelper.Rect = new Geometry.Rectangle(soldier.To2D(),
                    minion.ServerPosition.To2D(),
                    SpellConfig.Q.Width + 65);
                ...
                Global.Player.ManaPercent() > MenuConfig.Lane["W"].Value)
            {
                var minion = GameObjects.EnemyMinions.FirstOrDefault(x =>
                    x.Distance(Global.Player) <= SpellConfig.W.Range && x.MaxHealth > 10 && x.IsValid && !x.IsDead);
                ...
                if (GameObjects.EnemyMinions.Count(x => circle.Center.Distance(x.ServerPosition) <= circle.Radius) >=
                    3 &&
                    Global.Player.GetSpell(SpellSlot.W).Ammo > 1)
                {
            ...
            if (SpellConfig.E.Ready &&
                MenuConfig.Lane["E"].Enabled &&
                Global.Player.ManaPercent() > MenuConfig.Lane["E"].Value)
            {
                foreach (var soldier in SoldierManager.Soldiers)
                {
                    AzirHelper.Rect = new Geometry.Rectangle(Global.Player.ServerPosition.To2D(),
