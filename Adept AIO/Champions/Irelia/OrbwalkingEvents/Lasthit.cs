    ...
    class Lasthit
    {
        public static void OnUpdate()
        {
            if (!SpellConfig.Q.Ready ||
                !MenuConfig.Clear["Lasthit"].Enabled ||
                MenuConfig.Clear["Lasthit"].Value > Global.Player.ManaPercent())
            {
                return;
            }
            ...
            {
                if (!minion.IsValid ||
                    minion.Distance(Global.Player) < Global.Player.AttackRange ||
                    MenuConfig.Clear["Turret"].Enabled && minion.IsUnderEnemyTurret())
                {
                    continue;
                }
