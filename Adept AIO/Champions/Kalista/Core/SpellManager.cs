    ...
    class SpellManager
    {
        ...
        public static void CastQ(Obj_AI_Base target, int minHit = -1)
        {
            ...
            if (minHit == -1)
            {
                ...
                var killableCount = GameObjects.EnemyMinions.Count(x => x.HasBuff("kalistaexpungemarker") &&
                                                                        rect.IsInside(x.ServerPosition.To2D()) &&
        ...
        {
            var loc = Locations.OrderBy(x => x.Distance(Global.Player)).FirstOrDefault(x => x.Distance(Global.Player) > 1300 && x.Distance(Global.Player) <= W.Range);
            if (W.Ready && Global.Player.GetSpell(SpellSlot.W).Ammo >= 2)
            {
                W.Cast(loc);
            }
        }
