...
namespace Adept_AIO.Champions.LeeSin.Update.OrbwalkingEvents.Insec
{
    internal class Insec : IInsec
    {
        ...
        private Vector3 InsecPosition => target.ServerPosition + (target.ServerPosition - GetTargetEndPosition()).Normalized() * DistanceBehindTarget();
        ...
        public void OnKeyPressed()
        {
            ...
            if (SpellConfig.Q.Ready)
            {
                ...
                {
                    if (QLast && InsecPosition.Distance(GlobalExtension.Player) <= GetGapcloseDistance() && (SpellConfig.W.Ready || SummonerSpells.Flash != null && SummonerSpells.Flash.Ready &&
            ...
            {
                if (SpellConfig.Q.Ready && !SpellConfig.IsQ2() && target.Distance(ObjectManager.GetLocalPlayer()) <= GetGapcloseDistance() && !QLast)
                {
                    return;
                }
                ...
                else if (SummonerSpells.Flash != null && SummonerSpells.Flash.Ready && InsecPosition.Distance(GlobalExtension.Player) < 600 + 425 && Environment.TickCount - LastQTime > 650)
                {
                    WardFlash = true;
            ...
            else if (InsecKickValue == 0 && SummonerSpells.Flash != null && SummonerSpells.Flash.Ready && InsecPosition.Distance(GlobalExtension.Player) <= 425 && !(Environment.TickCount - _wardTracker.LastWardCreated <= 1500 && !WardFlash))
            {
                SummonerSpells.Flash.Cast(InsecPosition);
            }
            else if (!target.IsValidTarget(SpellConfig.R.Range) || Environment.TickCount - _wardTracker.LastWardCreated > 1000 || GlobalExtension.Player.Distance(InsecPosition) >= 235)
            {
                return;
            }
